  ...
  return (
    <main className="min-h-screen bg-neutral-950 text-white grid place-items-center px-6 py-12">
      <section className="w-full max-w-md rounded-3xl border border-white/10 bg-white/[0.04] p-8 shadow-2xl">
        ...
        <p className="mt-2 text-sm text-white/60">Create your account first. A paid plan will be required before real video generation.</p>
        ...
        <form action={signUp} className="mt-6 space-y-4">
          ...
          <button className="w-full rounded-xl bg-violet-500 px-4 py-3 font-semibold hover:bg-violet-400">Create account</button>
        </form>

        <p className="mt-5 text-sm text-white/60">Already registered? <Link href="/login" className="text-violet-300 hover:text-violet-200">Sign in</Link></p>
      </section>
    </main>
  );
}
